<template>
	<div v-loading="loading" v-show="visible" class="wrap-drag-verify-img-chip" @click.stop="doClick">
		<drag-verify-img-chip ref="dragVerify" :imgsrc="imgsrcArr[imgsrcIndex]" :isPassing.sync="isPassing"
			:showRefresh="false" :width="300" :diffWidth="5" text="请按住滑块拖动" progressBarBg="#FED88A"
			completedBg="#FED88A" successText="验证通过" handlerIcon="el-icon-d-arrow-right"
			successIcon="el-icon-circle-check" refreshIcon="el-icon-refresh-right" @refresh="refreshChip"
			@passcallback="passcallback" @passfail="passfail">
