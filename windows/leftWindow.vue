<template>
	<scroll-view scroll-y class="main left-window" :class="{ 'left-window-unfold': !isCollapse }">
		<el-menu :default-active="activeMenu" :collapse="isCollapse" :collapse-transition="false" background-color="#304156" text-color="#FFFFFF" class="side-menu">
			<el-menu-item index="index" @click="navigateTo('index', '首页', '/pages/index/index')">
				<i class="el-icon-s-home"></i>
				<span slot="title">首页</span>
			</el-menu-item>
			<el-menu-item index="richeditor" @click="navigateTo('richeditor', '富文本编辑器', '/pages/richeditor/richeditor', false)">
				<span slot="title">富文本编辑器</span>
			</el-menu-item>
			<el-menu-item index="form" @click="navigateTo('form', '表单', '/pages/form/form')"><span slot="title">表单</span></el-menu-item>
			<el-menu-item index="table" @click="navigateTo('table', '表格', '/pages/table/table')"><span slot="title">表格</span></el-menu-item>
				...
				<el-submenu v-if="!menu.pid" :index="menu.id" :key="menu.id">
					...
					<el-menu-item-group>
						<template v-for="child in menus">
							<el-menu-item v-if="child.pid == menu.id" :index="child.id" :key="child.id" @click="navigateTo(child.id, child.text, child.url, child.keepAlive)">
								{{ child.text }}
							</el-menu-item>
						</template>
					</el-menu-item-group>
