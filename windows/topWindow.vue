<template>
	<view class="top-window flex">
		<view v-if="!isXS" class="nav-header flex flex-justify-center flex-align-center" :style="{ minWidth: (isCollapse ? 64 : 230) + 'px' }">
			<el-avatar v-if="isCollapse" size="small" src="https://undsky.com/public/img/logo.png"></el-avatar>
			<view v-else>mc-uniAdmin</view>
		</view>
		<view @click="toggleSidebar" class="toggle-sidebar flex flex-justify-center flex-align-center"><i :class="[isCollapse ? 'el-icon-s-unfold' : 'el-icon-s-fold']"></i></view>
		<view class="flex flex-align-end">
			<el-tabs
				:style="{ width: 'calc(100vw - ' + (isXS ? 80 : isCollapse ? 254 : 420) + 'px)' }"
				tab-position="bottom"
				v-model="activeTab"
				type="card"
				@tab-click="tabClick"
			...
			>
				<el-tab-pane v-for="item in tabs" :key="item.name" :label="item.title" :name="item.name" :closable="item.closable"></el-tab-pane>
			</el-tabs>
		</view>
...
export default {
	...
	methods: {
		...
		tabClick({ name }) {
			const { url, keepAlive } = this.$utils.find(this.tabs, tab => tab.name === name);
