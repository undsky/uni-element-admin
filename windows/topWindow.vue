<template>
	<view class="top-window flex">
		<view v-if="!isXS" class="nav-header flex flex-justify-center flex-align-center"
			...
			<view class="text-weight-bold">MC</view>
			<!-- <el-avatar size="small" src="https://undsky.com/public/img/logo.png"></el-avatar> -->
			<view v-if="!isCollapse">-framework</view>
		</view>
		<view @click="toggleSidebar" class="toggle-sidebar flex flex-justify-center flex-align-center">
	...
	export default {
		...
		methods: {
			...
			tabClick({
				...
				const {
					url
				} = this.$util.find(this.tabs, tab => tab.name === name);
